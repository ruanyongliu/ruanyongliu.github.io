...
#### 索引 & Handoff
索引是segment的创建机制，handoff是segment发布到Historical机制。索引端工作机制如下:
1. 一个新的索引任务开始运行，创建一个新的segment。并且必须在创建前决定标识符。对于追加数据的任务(如Kafka、或追加模式的索引任务)，他会通过调用Overload进程的`allocate`API潜在地在现有的segment集合添加一个的分区。对于一个重写任务(如Hadoop任务，或者不是追加模式的索引任务)，他会通过在时间段上加锁，然后创建一个新的版本号和一个新的segment集合。
2. 如果是一个实时索引任务(如Kafka)，segment是可以立即被查询的。他是可用但未发布的。
3. 当索引任务完成给segment读入数据之后，推送至深度存储，添加新记录到元数据存储来发布。
4. 最后，如果是一个实时索引任务，他会等来Historical进程加载segment，否则segment会立刻存在。
...
- 使用Kafka索引，分区固定是基于Kafka的分区并且不能配置。你可以在Kafka的生产者端进行配置。
...
perfect roll-up模式包括了额外的预处理步骤，以便如果ingestionSpec没有指定，在数据导入之前也能确定时间段和shardSpecs。预处理步骤通常会扫描整个输入数据，因此会增加导入时间。[Hadoop索引](/TODO)任务通常使用perfect roll-up模式。

相反，best-effort roll-up模式不需要任何预处理步骤，但是导入数据集大小会比perfect roll-up大。所有的[流式索引(如Kafka索引)](/TODO)使用这种模式。

最后，[本地索引任务](/TODO)支持两种模式，你可以根据你的应该选择合适的方案。
