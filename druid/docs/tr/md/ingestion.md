...
- **发布的，不可用，不可使用的**：segment已经发布到深存储和元数据存储，但是不活跃(被重写或者删除了)。他们处于休眠状态，需要的话可以手动复活(特别地：将使用状态设为可以使用)
- **未发布的，可用的**：segment还在被Druid导入任务创建中。这里包括了所有实时的还没被Historical处理的数据。这个状态的segment可能有或者没有被复制。如果所有副本被丢弃了，segment需要从导入源重新创建，如果可以(Kafka是可以，并且自动的；S3/HDFS可以通过重新启动作业；Tranquility是不可以的，这种情况下数据就会被丢失)。
...
#### Roll-up 模式
Druid支持两种roll-up模式，perfect roll-up和best-effort roll-up。在perfect roll-up模式，Druid保证数据在导入时完全聚合。作为对比，best-effort roll-up模式下，数据可能不会完全聚合，因此结果可能会有若干segment包含应该属于在perfect rollup下相同的行，因为他们具有相同的维度，时间也落在同一个时间段上。  
perfect roll-up模式包括了额外的预处理步骤，以便如果ingestionSpec没有指定，在数据导入之前也能确定时间段和shardSpecs。预处理步骤通常会扫描整个输入数据，因此会增加导入时间。[Hadoop indexing](/TODO)任务通常使用perfect roll-up模式。  
相反，best-effort roll-up模式不需要任何预处理步骤，但是导入数据集大小会比perfect roll-up大。所有的[流式indexing(如Kafka indexing)](/TODO)使用这种模式。  
最后，[本地index任务](/TODO)支持两种模式，你可以根据你的应该选择合适的方案。
